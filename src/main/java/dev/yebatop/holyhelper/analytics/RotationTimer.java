...
public final class RotationTimer {
     ...
     * <p>
     * Но если сроки совпали, порознь их показывать нельзя: два разных заголовка над
     * одним и тем же числом читаются как две независимые проверенные величины, а на
     * деле это одно наблюдение, показанное дважды. Причин совпадения мод не знает —
     * то ли сервер обновляет обе группы разом, то ли пишет один срок на все товары, —
     * и разбираться в них здесь незачем: в обоих случаях правда одна, и строка нужна
     * одна. Разойдутся сроки — строки сами вернутся обратно.
     * <p>
