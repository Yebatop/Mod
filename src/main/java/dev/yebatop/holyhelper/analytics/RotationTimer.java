...
public final class RotationTimer {
    ...
    /**
     * Одни ли это часы на обе группы.
     * <p>
     * Остаток мод берёт из подсказки товара — «Обновление товаров через», — и строка
     * эта у каждого товара своя. Пока обе группы дают разные сроки, часов двое,
     * и показывать их порознь честно.
     * <p>
     * Но если сроки совпали, порознь их показывать нечего: два заголовка над одним
     * и тем же числом занимают вдвое больше места, ничего не добавляя. Совпадение
     * это настоящее — проверено в игре, сервер и правда показал обеим группам один
     * остаток, — так что ошибки разбора здесь нет и прятать нечего. Поэтому строка
     * одна, но подписана она обеими группами сразу: молчать о том, что групп две,
     * тоже нельзя — они разойдутся после ближайшей ротации, и тогда строки
     * вернутся обратно сами.
     * <p>
     * Минута допуска потому, что сроки читаются из разных подсказок и снимаются не
     * в одну и ту же миллисекунду.
     */
    public boolean singleClock() {
        Instant regular = regularAt;
        Instant special = specialAt;
        if (regular == null || special == null) {
            return false;
        }
        return Duration.between(regular, special).abs().compareTo(Duration.ofMinutes(1)) <= 0;
    }
